...
if __name__ == "__main__":
    ...
    Solution:
        y(x) = e^x * (2*sin(x) + cos(x))
        z(x) = e^x * (2*cos(x) - sin(x))
    """
    ode = BVP(odes=("y' = y + z", "z' = -y + z"),
              interval=(0, 2),
              bcs=("y(0) = 1", "z(0) = 2"))
    ...
    Solution:
        ...
        z(x) = 
    """
    ode = BVP(odes=("y'' + z'' = -np.sin(x)", "z'' - y = np.cos(x)"),
              interval=(0, 1),
    ...
    """
    Solving for y(x) and parameter k:
        y''(x) + k**2 * y = 0
